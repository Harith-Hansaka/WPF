...
namespace UNDAI.MODELS.SLAVE
{
    public class ConnectionSlave
    {
        ...
        ReceiveDataSlave _receiveDataSlave;
        int tempRandomNum;
        App app;
        ...
        public async void ConnectClick(MainPageSlaveViewModel mainPageSlaveViewModel, int Identifier)
        {
            ...
            serverIP = IPAddress.Parse(currentServerIP);
            iPEndPoint = new IPEndPoint(serverIP, serverPort);
            app._systemSettingSlaveViewModel.UndaiIPAddress = currentServerIP;
            ...
            if (_mainPageSlaveViewModel.Connected == "CONNECT" && isBtnPointerReleased)
            {
                if (client == null)
                {
                    ...
                    try
                    {
                        ...
                        connected = true;
                        _mainPageSlaveViewModel.Connected = "CONNECTED";
                        // Start receiving data from server
                        _receiveDataSlave = new ReceiveDataSlave(connected, client, _mainPageSlaveViewModel, this);
                        stillConnected = true;
                        _ = ConnectionCheck();
                    }
                    catch (Exception ex)
                    {
                        client = null;
                        _mainPageSlaveViewModel.Connected = "CONNECT";
        ...
        public async Task ConnectionCheck()
        {
            ...
            {
                if (_mainPageSlaveViewModel.MainPageSlaveModel.messageReceivedCount > 0)
                {
                    int tempRandomNum = _mainPageSlaveViewModel.MainPageSlaveModel.randomNumber;
        ...
        public void CloseConnection()
        {
            try
            {
                ...
                if (_receiveDataSlave != null)
                {
                    _receiveDataSlave.StopReceiving(); // Ensure StopReceiving is awaited if it's async
                    _receiveDataSlave = null;
                }
                connected = false;
                isLongPressed = false;
                stillConnected = false;
                _mainPageSlaveViewModel.Connected = "CONNECT";
                _mainPageSlaveViewModel.SlaveAlarmData = "Connection closed.";
            }
            catch (Exception ex)
            {
                _mainPageSlaveViewModel.SlaveAlarmData = $"Error closing connection: {ex.Message}";
            }
        }
